 ...
 * Class BidRequestAppContent
 * @package Ldar\RtbSpecLib\bid\request
 */
class BidRequestAppContent extends Model
{
    /**
     ...
     * @var BidRequestAppContentProducer|null
     */
    protected ?BidRequestAppContentProducer $producer = null;
    ...
    public function getProducer(): BidRequestAppContentProducer
    {
    ...
    {
        if ($values instanceof BidRequestAppContentProducer) {
            $this->producer = $values;
        }
        if (is_array($values)) {
            $this->producer = new BidRequestAppContentProducer();
